...
const DriverBacked = () => {
  ...
  async function claimPoints() {
    ...
    const response = await fetch(`/api/driverpoints?token=${token}`);
  ...
  return (
    <div className="px-14 w-full flex items-center justify-center">
      <div>
        <Image
          src={require(`../../public/img/drivers/${user.driverwager.driver}.png`)}
          width={350}
          height={280}
        />
        <p className="my-3 text-white font-semibold text-xl">
          ...
          <span className="capitalize text-redOne">
            {user.driverwager.driver.replace('-', ' ')}
          </span>{' '}
          with your{' '}
        ...
        {RACE.claim < Math.round(new Date().getTime() / 1000) ? (
          ...
          <div>
            <a
              href={`https://twitter.com/intent/tweet?text=I+am+backing+${user.driverwager.driver
                .replace('-', ' ')
                .replace(/(^\w|\s\w)/g, (m: any) => m.toUpperCase())}+for+the+${
                RACE.name
              }+${RACE.emoji}+with+my+${garage
                .find((item: any) => item.itemId == user.driverwager.itemId)
                .name.replace(
                  '#',
                  '%23'
